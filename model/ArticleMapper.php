...
class ArticleMapper
{
    ...
    public function getAllArticles(): array
    {
        try {
            ...
            foreach ($result as $rawArticle) {
                array_push($article, new Article($rawArticle['id_article'], $rawArticle['title'], $rawArticle['content'], $rawArticle['owner']));
            }
            /*ob_start();
            var_dump($article);
    ...
    public function getArticleById(int $id_article)
    {
        try {
            ...
            return new Article($result['id_article'], $result['title'], $result['content'], $user, null, $result['auditcd'], $result['auditmd']);
            /*ob_start();
            var_dump($article);
            error_log(ob_get_clean());*/
        } catch (PDOException $e) {
